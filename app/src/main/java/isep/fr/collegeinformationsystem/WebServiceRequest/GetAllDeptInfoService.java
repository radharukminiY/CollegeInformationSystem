...
public class GetAllDeptInfoService {
    ...
    public GetAllDeptInfoService(Context context, final String deptShortName, ServerResponseListener aServerResponseListener) {
    ...
    public void GetAllDeptInfoService() {
        StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
        StrictMode.setThreadPolicy(policy);

        StringRequest stringRequest = new StringRequest(
                Request.Method.POST,
                Constants.GET_ALL_DEPT_URL,
                            ...
                            if (status.equals("0")) {
                                ...
                                if (jsonArray.length() != 0) {
                                    for (int i = 0; i < jsonArray.length(); i++) {
                                        ...
                                        String deptName = staffObj.getString(Constants.DEPT_NAME);
                                        String deptShortName = staffObj.getString(Constants.DEPT_SHORT_NAME);
                                        String deptSubjects = staffObj.getString(Constants.DEPT_SUBJECTS);
                                        String deptSylDoc = staffObj.getString(Constants.DEPT_SYLL_DOC);
            ...
            protected Map<String, String> getParams() {
                ...
                params.put(Constants.DEPT_SHORT_NAME, deptShortName);
